#!/bin/bash
# 宝塔终端一键迁移脚本 - Phase 5 数据库

mysql -h117.24.15.9 -P3306 -uxingyunpan -pRMGdedeMoMnMc xingyunpan << 'SQLEOF'
...
SET @exist := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema = DATABASE() AND table_name = 'user_files' AND index_name = 'idx_file_name');
SET @sqlstmt := IF(@exist = 0, 'CREATE INDEX idx_file_name ON user_files(file_name)', 'SELECT ''Index exists''');
PREPARE stmt FROM @sqlstmt; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @exist := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema = DATABASE() AND table_name = 'user_files' AND index_name = 'idx_modified_at');
SET @sqlstmt := IF(@exist = 0, 'CREATE INDEX idx_modified_at ON user_files(updated_at)', 'SELECT ''Index exists''');
PREPARE stmt FROM @sqlstmt; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @exist := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema = DATABASE() AND table_name = 'user_files' AND index_name = 'idx_file_size');
SET @sqlstmt := IF(@exist = 0, 'CREATE INDEX idx_file_size ON user_files(file_size)', 'SELECT ''Index exists''');
PREPARE stmt FROM @sqlstmt; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SELECT '✅ Phase 5 迁移完成！' AS status;
SQLEOF
