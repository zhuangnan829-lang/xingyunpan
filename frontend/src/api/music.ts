...
export async function listMusic(params: MusicListRequest = {}): Promise<MusicListResponse> {
  try {
    const response = await request.get<any>('/api/v1/music', { params });
    const normalized = normalizeMusicListResponse(response, params);
    if (normalized.files.length > 0) {
      return normalized;
    }
    return listMusicWithSearchFallback(params);
  } catch {
    return listMusicWithSearchFallback(params);
  }
}

function normalizeMusicListResponse(response: any, params: MusicListRequest): MusicListResponse {
  return {
    files: (response.files || []).map((item: any) => ({
      ...
      content_type: item.content_type || item.mime_type || 'audio/mpeg',
...
async function listMusicWithSearchFallback(params: MusicListRequest): Promise<MusicListResponse> {
  ...
  const files = response.files.filter(isAudioFile).sort((a, b) => {
